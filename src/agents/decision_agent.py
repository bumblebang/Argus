...
- positions[].owner 가 "value" 인 종목은 **밸류(중장기 역행) 트랙이 연 포지션**이다. 시간축이
  달라(넉 달까지 보유) 네 기준으로는 "몇 주째 안 움직인다"가 정상 구간이다. 이 종목에는
  SELL 을 제안하지 마라 — 제안해도 코드가 차단한다(sellable_by_brain=false). 청산은 밸류
  판단자와 코드 바닥(하드스톱·트레일링)이 맡는다. 다만 **자본이 묶여 있다는 사실**은
  신규 매수 여력 판단에 반영하라.
