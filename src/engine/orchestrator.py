"""Watch 런타임 조립 (Phase 1) — 스레드/워커 배선.

CLI 파싱은 scripts/watch.py (또는 argus watch). 여기는 run_from_args(args) 만.
...
loop_ref: WatchLoop 와 Brain 의 순환 의존을 깨는 지연 홀더(illiquid/breadth).
"""
...
def _build_disclosure_watcher(cfg, store, brain,
    ...
    if wcfg.get("earnings_consensus", True):
        read_cal = _earnings_calendar_reader(ROOT / "data" / "earnings_calendar.json")
...
def run_from_args(args) -> int:
    ...
    wcfg = cfg.raw.get("watch", {})
    store = Store(run_cfg.get("db_path", _paths.rel("db")))
    ...
    heartbeat = str(_paths.resolve(
        "watch_hb", configured=run_cfg.get("heartbeat_path")))
